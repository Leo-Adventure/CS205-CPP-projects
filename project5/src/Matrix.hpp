...
Matrix<T>::Matrix(int row, int col, int channel, T* nums, int ROIBeginRow, int ROIBeginCol, int ROIRow, int ROICol, int* ref_count, int span ){
    ...
    if(nums == nullptr){
        ...
        if(this -> nums == nullptr){
            std::cerr << "FAILURE TO ALLOCATE MEMORY in line " << __LINE__ << " in file " << __FILE__ << std::endl;
            exit(EXIT_FAILURE);
        }
...
Matrix<T>::Matrix(int row, int col, int channel, std::ifstream & fin, int size){
    ...
    memset(this -> nums, 0, sizeof(T) * size);
    int pos = 0;
...
Matrix<T> Matrix<T>::operator+(const Matrix<T> &m)const{
    ...
    memset(mat.nums, 0, sizeof(T) * ele_num);
...
Matrix<T>& Matrix<T>::operator+=(const Matrix<T> &mat){
    ...
    if(this -> ref_count[0] == 1){//如果引用次数为1，则直接改变原数组的元素值
        ...
        memset(n, 0, sizeof(T) * row * col * channel);
        int pos = 0;
...
Matrix<T> Matrix<T>::operator/(T num) const{
    ...
    mat.span = span;
    mat.nums = new T[row * col * channel];
    memset(mat, 0, sizeof(T) * row * col * channel);
...
T & Matrix<T>::operator()(int r, int c, int ch){
    ...
    if(ch < 1 || ch > channel){
        std::cerr << "In file " << __FILE__ << " in line " << __LINE__ << " the input channel is " << ch << " but the source channel is " << channel << std::endl;
        std::cerr << "The input of channel should be between 1 and " << channel << std::endl;
        exit(EXIT_FAILURE);
    }
    // std::cout << "nums[r * span + c * channel + ch - 1] = " << nums[r * span + c * channel + ch - 1] << std::endl;
...
Matrix<T>& Matrix<T>::append(T *nums, int r, int c, int ch){
    ...
    T * arr = new T[src_ele_num + tar_ele_num];//创建新元素数组，大小为两个数组大小之和
     memset(arr, 0, sizeof(T) * (src_ele_num + tar_ele_num));
    int pos = 0;
    for(int i = 0; i < row; i++){
        for(int j = 0; j < col; j++){
            for(int ch = 1; ch <= channel; ch++){
...
Matrix<T> Matrix<T>::merge_vertical(const Matrix<T>& mat) const {
    ...
    memset(res.nums, 0, sizeof(T) * (res.row * col * channel));
...
Matrix<T> Matrix<T>::merge_horizontal(const Matrix<T> & mat)const {
    ...
    memset(res.nums, 0, sizeof(T) * (row * res.col * channel));
